"""Admin authentication, dashboard, broadcast, analytics, logs."""
...
# Bootstrap usernames — once they /start the bot, their numeric id is captured
# and persisted as admin in the DB. After that, only numeric ids are checked.
BOOTSTRAP_ADMIN_USERNAMES = {"iampranjal09", "radheshyam001"}

BROADCAST_BATCH_SIZE = 25
BROADCAST_DELAY_SEC = 0.05  # ~20 messages/sec — safely under Telegram's 30/sec limit
PROGRESS_EVERY = 25

# Per-admin transient state (in-memory)
...
def get_admin_ids() -> list[int]:
    with connect() as con:
        return [r["user_id"] for r in con.execute("SELECT user_id FROM users WHERE is_admin = 1")]
...
async def admin_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not user or not is_admin_user_id(user.id):
        # Silent for non-admins — dashboard must be invisible
        await update.message.reply_text("⛔ Access denied.")
        log_event("WARN", "ADMIN", "Non-admin attempted /admin", actor_id=user.id if user else None)
        return
...
async def reply_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(parts) < 3:
        await update.message.reply_text("Usage: `/reply CRIC-1001 your reply text`", parse_mode=ParseMode.MARKDOWN)
        return
    ticket_num = parse_ticket_id(parts[1])
    if ticket_num is None:
        await update.message.reply_text("Invalid ticket id. Example: `CRIC-1001`", parse_mode=ParseMode.MARKDOWN)
        return
    ...
    try:
        await context.bot.send_message(
            ...
            text=(
                f"💬 *Reply on ticket {format_ticket_id(ticket_num)}*\n\n"
                f"{body}"
            ),
            parse_mode=ParseMode.MARKDOWN,
        )
        ...
        log_event("INFO", "ADMIN", f"Replied to {format_ticket_id(ticket_num)}", actor_id=user.id)
    except TelegramError as exc:
        await update.message.reply_text(f"⚠️ Could not deliver: {exc}")


async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """/status CRIC-1001 RESOLVED"""
    user = update.effective_user
    if not user or not is_admin_user_id(user.id):
        await update.message.reply_text("⛔ Access denied.")
        return
...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """/broadcast <message> — opens a confirmation prompt."""
    user = update.effective_user
    if not user or not is_admin_user_id(user.id):
        await update.message.reply_text("⛔ Access denied.")
        return
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "adm_home":
        await query.edit_message_text(
            DASHBOARD_TEXT, reply_markup=dashboard_keyboard(), parse_mode=ParseMode.MARKDOWN
        )
    elif data == "adm_tickets":
        await query.edit_message_text(
            _render_tickets_panel(), reply_markup=back_to_dashboard_kb(), parse_mode=ParseMode.MARKDOWN
        )
    elif data == "adm_users":
        await query.edit_message_text(
            _render_users_panel(), reply_markup=back_to_dashboard_kb(), parse_mode=ParseMode.MARKDOWN
        )
    elif data == "adm_analytics":
        await query.edit_message_text(
            _render_analytics(), reply_markup=back_to_dashboard_kb(), parse_mode=ParseMode.MARKDOWN
        )
        ...
        await query.edit_message_text(
            _render_ai_panel(), reply_markup=_ai_keyboard(), parse_mode=ParseMode.MARKDOWN
        )
    elif data == "adm_ai_toggle":
        new_val = "0" if get_setting("ai_enabled", "1") == "1" else "1"
        set_setting("ai_enabled", new_val)
        log_event("INFO", "ADMIN", f"AI {'enabled' if new_val == '1' else 'disabled'}", actor_id=user.id)
        await query.edit_message_text(
            _render_ai_panel(), reply_markup=_ai_keyboard(), parse_mode=ParseMode.MARKDOWN
        )
    elif data == "adm_system":
        await query.edit_message_text(
            _render_system_panel(), reply_markup=_system_keyboard(), parse_mode=ParseMode.MARKDOWN
        )
        ...
        await query.edit_message_text(
            _render_logs(), reply_markup=back_to_dashboard_kb(), parse_mode=ParseMode.MARKDOWN
        )
...
async def broadcast_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "bcast_confirm":
        ...
        if not message:
            ...
            return
        await _run_broadcast(query, context, user.id, message)


# ---------------------------------------------------------------------------
# Broadcast engine (rate-limited, batched, retry-aware)
# ---------------------------------------------------------------------------


async def _run_broadcast(query, context: ContextTypes.DEFAULT_TYPE, admin_id: int, message: str) -> None:
    with connect() as con:
    ...
    sent = 0
    failed = 0
    log_event("INFO", "BROADCAST", f"Broadcast started → {total} users", actor_id=admin_id)
...
def _render_users_panel() -> str:
    ...
    if not recent:
        ...
        for r in recent:
            ...
            lines.append(f"• `{r['user_id']}` @{uname} — {r['total_requests']} req — _{ts}_")
    return "\n".join(lines)
...
def _render_system_panel() -> str:
    return (
        ...
        "• Bot version: enterprise v1.0\n"
        "• Storage: SQLite (cricway.db)\n"
    )
