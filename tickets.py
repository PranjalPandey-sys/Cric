...
from database import connect, format_ticket_id, log_event, now_iso
...
def create_ticket(user_id: int, subject: str, priority: str = "MEDIUM") -> int:
    """Create a ticket and return its numeric id (e.g. 1001)."""
    ...
    with connect() as con:
        cur = con.execute(
            "INSERT INTO tickets (user_id, subject, status, priority, handled_by, created_at, updated_at) "
            "VALUES (?, ?, 'OPEN', ?, 'PENDING', ?, ?)",
            (user_id, subject, priority, ts, ts),
        )
        ticket_id = int(cur.lastrowid)
        con.execute(
            "INSERT INTO ticket_replies (ticket_id, sender_id, sender_role, message, created_at) "
            "VALUES (?, ?, 'USER', ?, ?)",
...
def add_reply(
    ticket_id: int, sender_id: Optional[int], sender_role: str, message: str
) -> None:
    ts = now_iso()
    with connect() as con:
        con.execute(
...
def assign_admin(ticket_id: int, admin_id: int) -> None:
    with connect() as con:
        con.execute(
            "UPDATE tickets SET assigned_admin = ?, status = 'IN_PROGRESS', updated_at = ? "
            "WHERE ticket_id = ?",
...
def auto_close_stale(days: int = 7) -> int:
    """Auto-close OPEN tickets with no activity for `days` days."""
    cutoff = now_iso()
    with connect() as con:
        cur = con.execute(
            "UPDATE tickets SET status = 'RESOLVED', handled_by = 'SYSTEM', updated_at = ? "
